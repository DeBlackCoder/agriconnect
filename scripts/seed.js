// Node.js compatible seed script
const mongoose = require('mongoose');

// Read from .env.local manually if needed
...
async function seed() {
  try {
    ...
    farmers.forEach(f => console.log(`  - ${f.fullName} (${f.email})`));
    ...
    if (existingCategories === 0) {
      ...
      category = await Category.findOne();
      console.log(`✅ Using existing categories (${existingCategories} found)`);
    }
    ...
    console.log(`Current products in DB: ${existingProducts}`);
    ...
    console.log(`\n📊 Final count: ${finalCount} products in database`);
    
    if (finalCount !== createdProducts.length) {
